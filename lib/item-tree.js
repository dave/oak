...
ItemObject.prototype.showInList = function(type, change) {
	if (this.attribute('_enabled') === false) {
		/**
		 * If the live version is disabed...
		 */
		if (this.attribute('_enabled', change) === true){
			/**
			 * Show the item if this change enabled it
			 */
		}
		if (change != null && this.attribute('_created', change) === change.id()){
			/**
			 * Show the item if this change created it
			 */
		}
		else if (this.selected() || _.contains(Session.get('selected_ancestors'), this.id())) {
			/**
			 * TODO: Show the item if we're currently editing it, or a descendant
			 */
		}
		else if (Session.equals('show_deleted_items', true)) {
			/**
			 * TODO: Show the item if we're showing deleted items
			 */
		}
		else {
			return false;
	...
	//if (type == 'tree' && this.parent() && this.parent().type().list) {
	//	/**
	//	 * In the tree view, only show an item in a list if you're editing it, or a child of it.
	//	 */
	//	return this.selected() || _.contains(Session.get('selected_ancestors'), this.id())
	//}
	return true;
}
