 ...
 * @returns {ItemObject}
 */
ItemObject.prototype.parent = function(returnPlainId) {
	if (returnPlainId === $id) {
		return this.data().parent;
	}
	return Item(this.data().parent);
}
 ...
 * @returns {ItemObject}
 */
ItemObject.prototype.select = function(keypress) {
	...
	if (keypress) {
		...
		//tally('active_items', this.childIds(), true);
...
/*
ItemObject.prototype.childIds = function() {
	var ids = [];
	_.each(this.children().array(), function(child){
		ids.push(child.id());
	});
	return ids;
}
...
ItemObject.prototype.visibleChildren = function(change, name) {
	...
	if (change != null) {
		query['$or'] = [{enabled: true}, {created: change.id()}];
	}
	else {
		query['enabled'] = true;
	}
