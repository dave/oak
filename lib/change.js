...
if (Meteor.isClient) {
	Meteor.startup(function () {
		Deps.autorun(function(){
			Session.get('selected_change');
			observeTweaks();
		});
	});
}

var observeTweaks = function(){
	if (currentChange()) {
		currentChange().tweaks().cursor().observe(
			{
				added: function(doc)
				{
					Tweak(doc).item().updateDirtyChildrenOnAncestors(currentChange());
				}
			}
		);
	}
...
ChangeObject.prototype.firstTweaks = function() {
	return new TweakCollection(Tweaks.find({change: this.id(), order: 0}));
}

ChangeObject.prototype.tweaks = function() {
	return new TweakCollection(Tweaks.find({change: this.id()}));
}
