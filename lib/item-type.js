...
ItemObject.prototype.listHoldsTypes = function() {
	return findAllMatchingTypes(this.listHolds());
}

ItemObject.prototype.childHoldsTypes = function(child) {
	if (this.type().children == null || this.type().children[child] == null)
		return [];
	return findAllMatchingTypes(this.type().children[child].types);
}

var findAllMatchingTypes = function(typeSpec) {
	var matchingTypes = [];
	_.each(typeSpec, function(typeName){
		_.each(types, function(type) {
			if (typeName == '*' ||
				type.name == typeName ||
				(type.is && _.contains(type.is, typeName))) {
